...
std :: pair<int, int> MouseInput ::  checkBoard(sf :: RenderWindow &window, GoBoard &goBoard){
    ...
    double spacing = (1.0 * (metaControls.ZONE_SIZE - 2 * metaControls.SHIFT_CONST) / (goBoard.boardSize - 1));
    for (int i = 0; i < goBoard.boardSize; ++i){
        for (int j = 0; j < goBoard.boardSize; ++j){
            int vertexX = metaControls.SHIFT_CONST + i * spacing;
            int vertexY = metaControls.SHIFT_CONST + j * spacing;
