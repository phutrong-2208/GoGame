...
void ScoreReveal :: setupScoreButton(std :: vector<Button> &button_list) {
    ...
    button_list.push_back(Button({width / 2.0f - 500 / 2.0f, height / 2.0f + 63}, {500, 50}, 
        19, 2, 
...
void ScoreReveal :: drawScoreReveal(sf :: RenderWindow &window, GoBoard& goBoard) {
    /*
    Deprecated code for drawing the board
    ...
    */
    ...
    overlay.setSize({metaControls.ZONE_SIZE * metaControls.ASPECT_RATIO, metaControls.ZONE_SIZE});
    sf :: Color overlayColor = metaControls.Color3 * sf :: Color(192, 192, 192) + sf :: Color(64, 64, 64);
    overlayColor.a = 192;
    ...
    panel.setPosition({(metaControls.ZONE_SIZE * metaControls.ASPECT_RATIO - panel.getSize().x) / 2.0f, (metaControls.ZONE_SIZE - panel.getSize().y) / 2.0f});
    panel.setFillColor(metaControls.Color2);
    customPanelRender(window, panel);
    ...
    drawText(window, FontLink, "White: " + std :: to_string(whiteScore), {width / 2.0f, height / 2.0f - 37});
    drawText(window, FontLink, "Black: " + std :: to_string(blackScore), {width / 2.0f, height / 2.0f + 13});
    drawText(window, FontLink, winner + reason, {width / 2.0f, height / 2.0f - 87});
}
